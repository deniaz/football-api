...
program.version('0.0.1')
       .option('-t, --teams', 'Scrape Teams')
       .option('-p, --players', 'Scrape Player')
       .option('-m, --matches', 'Scrape Matches')
       .parse(process.argv);

console.info('Scraping Football Data from ' + cfg.services.base_uri);
...
String.prototype.toSlug = function() {
    var diacritics = [
        ...
        str = this.replace('.', '').replace(/[0-9]/g, '').trim().replace(/\s+/g, '-').toLowerCase();
        ...
        docs.forEach(function(team) {
            ...
            request(uri, function(error, response, html) {
                if (!error) {
                    ...
                    console.log('All players stored.');
                } else {
                    console.error('HTTP Error: ' + error);
                    process.exit(1);
                }
...
if (program.matches) {
    console.log('Match Data has yet to be implemented.');
}

console.info('Scraping done, thanks and see you soon.')
